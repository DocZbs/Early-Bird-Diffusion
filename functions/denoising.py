    ...
    with torch.no_grad():
        ...
        for i, j in zip(reversed(seq), reversed(seq_next)):
            ...
            if 0 <= i < 240: 
                ...
                mask_index = 1 # medium model
            elif 440 <= i < 720:
                mask_index = 2 # smallest model
            else:
                mask_index = 3

            model_i = models[mask_index]
            et =model_i(xt, t)
...
def generalized_steps_dit(x, seq, model, b, y=None, **kwargs):
    with torch.no_grad():
        n = x.size(0)
        seq_next = [-1] + list(seq[:-1])
        x0_preds = []
...
def ddpm_steps_dit(x, seq, model, b, y=None, **kwargs):
    with torch.no_grad():
        n = x.size(0)
        seq_next = [-1] + list(seq[:-1])
        xs = [x]
